...
Current server/mfa.js and createApp routes use an explicitly configured server-only `MFA_ENCRYPTION_KEY` (32-byte hex or padded base64). No generated/default production master key exists. This protects enabled/pending authenticator secrets with AES-256-GCM; it does not encrypt the live SQLite business database. MFA applies to workspace accounts, including their administrator/staff/viewer roles, **not the separate platform master administrator**. Enrollment is per-account; no enforced organization-wide enrollment policy or SSO is claimed.
