...
| Readiness | `node scripts/production-check.mjs --help` exposes a private operator check. It reads the workspace without modification and outputs safe checks, not credentials. Missing durability, readback, restore, monitoring or retention evidence blocks readiness. | Actual operator-controlled evidence; runtime checks do not certify compliance, residency, provider SLA or buyer acceptance. |
