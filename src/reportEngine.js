...
const BASE={
 documentMigrationMappings:'reconciliationId sourceNamespace sourceDocumentId sourceRevisionId sourceVisibility originalDate documentId documentRevision collection recordId recordRevision requestedDocumentRevision operation importedCategory importedStatus importedVisibility importedEvidenceDate mime sha256 size sourceRevisionCount nativeRevisionCount sourceBytes nativeBytes actor importedAt',
 documentMigrationReconciliations:'reconciliationId sourceNamespace sourceDocumentId sourceRevisionId sourceVisibility originalDate documentId documentRevision collection recordId recordRevision requestedDocumentRevision operation importedCategory importedStatus importedVisibility importedEvidenceDate mime sha256 size sourceRevisionCount nativeRevisionCount sourceBytes nativeBytes actor importedAt',
 ...
 taskReminderSchedules:'taskId ownerId taskRevision ownerRevision remindAt status revision createdAt updatedAt nextAttempt attemptCount',
 taskReminderOutcomes:'reminderId reminderRevision status reason at retryAt',
 taskReminderInbox:'reminderId ownerId taskId taskRevision deliveredAt delivery',
 eventHelperAssignments:'userId eventId active revision updatedAt',
 eventHelperAccessHistory:'userId fromRevision toRevision beforeEventIds afterEventIds reason actorId actorName at',
 savedAudiences:'revision status name segmentLabels segmentMatch types preferences createdAt updatedAt',
 audienceRevisions:'audienceId revision status name segmentLabels segmentMatch types preferences reason actor at',
 ...
 correspondencePreparations:'templateId templateName templateKind templateRevision channel audienceId audienceName audienceRevision audienceSelectedCount organizationName recipientCount giftCount monetaryCents noncashCents delivery status actor at',
...
const NUMBERS=new Set(['recordRevision','requestedDocumentRevision','sourceRevisionCount','nativeRevisionCount','sourceBytes','nativeBytes','receiptLinkCount','sourceGiftRevision','acknowledgmentGiftRevision','eventRevision','fromRevision','toRevision','hours','originalHours','capacity','installments','allocationCount','revision','revisionCount','memberCount','size','latestSize','totalRows','createCount','mappedCount','errorRows','giftCount','columnCount','filterCount','groupCount','calculationCount','recipientCount','templateRevision','reportVersion','ratioNumerator','ratioDenominator','calendarYear','profileRevision','giftRevision','originalGiftRevision','seats','seatNumber','detailCount','fiscalStartMonth','grantRevision','ownerRevision','milestoneRevision','documentVersion','documentRevision','documentSize','tributeRevision']);
const DATES=new Set(['originalDate','importedEvidenceDate','importedAt','beforeCheckedInAt','afterCheckedInAt','beforeDate','afterDate','date','startDate','endDate','awardDate','deadline','reportDue','dueDate','startAt','endAt','clockIn','correctedAt','createdAt','updatedAt','voidedAt','evidenceDate','retentionUntil','at','committedAt','nextRun','scheduledAt','executedAt','nextActionDate','assignedAt','cancelledAt','issuedAt','checkedInAt','unlinkedAt','associatedDate','issueDate','asOf','documentEvidenceDate','completedDate','sourceGiftDate','giftDate','finalizedAt']);
...
const ADMIN_METADATA=new Set(['documentMigrationMappings','documentMigrationReconciliations','taskReminderSchedules','taskReminderOutcomes','taskReminderInbox','eventHelperAssignments','eventHelperAccessHistory','tributeNotificationWithdrawals','identityAliases','migrationBatches','migrationMappings','receiptProfiles','receiptProfileRevisions','workspaceUsers','workspaceSettings','auditHistory','tributes','tributeVersions','tributeNotifications','tributeNotificationFinalizations']);
