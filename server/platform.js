...
export function createPlatformApp({ rootDir, legacyDbPath, seedLegacy = true, tenantFactory, initialPlatformAdmin, mfaKey=process.env.MFA_ENCRYPTION_KEY, mfaClock=Date.now } = {}) {
  ...
  router.post('/tenants', (req, res) => {
    const body = tenantSchema.parse(req.body); if (production && body.dataMode === 'synthetic') fail(403, 'Synthetic tenants are not permitted in the production control plane.'); const id = randomUUID(); let createdApp;
    try {
  ...
  router.patch('/tenants/:id', (req, res) => { const body = z.object({ version: z.number().int().min(1), status: z.enum(['active', 'suspended']).optional(), aiEnabled: z.boolean().optional(), plan: z.enum(['trial', 'subscription', 'dedicated']).optional() }).strict().refine(v => v.status !== undefined || v.aiEnabled !== undefined || v.plan !== undefined, 'A lifecycle, plan or AI change is required').parse(req.body); const tenant = tx(() => { const old = lookup(req.params.id); if (!old) fail(404, 'Tenant not found'); if (old.version !== body.version) fail(409, 'Tenant changed; reload before saving'); const next = { status: body.status ?? old.status, aiEnabled: body.aiEnabled ?? Boolean(old.ai_enabled), plan: body.plan ?? old.plan }; if (old.status === 'suspended' && next.status === 'active') {if (!existsSync(tenantPath(old.id))) fail(503,'Workspace storage is unavailable. Restore it before resuming access.');const workspace=getTenantApp(old);workspace.locals.suppressTaskReminders?.();workspace.locals.suppressCommunicationWorkflows?.();workspace.locals.revokeAllSessions?.();} db.prepare('UPDATE tenants SET status=?,ai_enabled=?,plan=?,version=version+1,updated_at=? WHERE id=?').run(next.status, next.aiEnabled ? 1 : 0, next.plan, new Date().toISOString(), old.id); audit(req.platformAdmin.id, 'update_tenant', old.id, { previous: { status: old.status, aiEnabled: Boolean(old.ai_enabled), plan: old.plan }, next }); return publicTenant(lookup(old.id)); }); if (tenant.status === 'suspended' && existsSync(tenantPath(tenant.id))) {try {const workspace=getTenantApp(lookup(tenant.id));workspace.locals.suppressTaskReminders?.();workspace.locals.suppressCommunicationWorkflows?.();workspace.locals.revokeAllSessions?.();}catch {fail(503,'Workspace suspension is committed. Pending automation suppression could not finish; retry suspension before restoring access.');}} res.json({ tenant }); });
