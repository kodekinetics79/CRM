...
const legacyDbPath=process.env.DB_PATH||resolve(root,evaluator?'server/data/evaluator.sqlite':'server/data/foundation.sqlite');
const app=createPlatformApp({rootDir:process.env.PLATFORM_DATA_DIR||resolve(dirname(legacyDbPath),basename(legacyDbPath,'.sqlite')+'-platform'),legacyDbPath,seedLegacy:evaluator||process.env.NODE_ENV!=='production'||process.env.ALLOW_DEMO==='true',tenantFactory:options=>createApp(options)});
